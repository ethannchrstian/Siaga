...
const FLOOD = ["#dbe7eb", "#91adbb", "#527d94"];
const DROUGHT = ["#f0e1e2", "#ca999d", "#9b5961"];
...
function ramp(colors: string[], p: number): string {
  if (p < MONITORING_THRESHOLD) return colors[0];
  if (p < HIGH_RISK_THRESHOLD) return colors[1];
  return colors[2];
}
...
export function colorFor(
  ...
  if (flood >= MONITORING_THRESHOLD && drought >= MONITORING_THRESHOLD) {
    return Math.max(flood, drought) >= HIGH_RISK_THRESHOLD ? COMPOUND[1] : COMPOUND[0];
  }
...
export function mapStyleFor(
  ...
  if (mode === "gabungan" && flood >= MONITORING_THRESHOLD && drought >= MONITORING_THRESHOLD) {
    return {
      fill: peak >= HIGH_RISK_THRESHOLD ? COMPOUND[1] : COMPOUND[0],
...
> = {
  gabungan: [
    { swatch: FLOOD[1], outline: OUTLINE.flood, label: "Banjir · 50–79%" },
    { swatch: FLOOD[2], outline: OUTLINE.flood, label: "Banjir tinggi · ≥80%" },
    { swatch: DROUGHT[1], outline: OUTLINE.drought, label: "Cekaman · 50–79%" },
    { swatch: DROUGHT[2], outline: OUTLINE.drought, label: "Cekaman tinggi · ≥80%" },
    { swatch: COMPOUND[1], outline: OUTLINE.compound, label: "Keduanya dipantau" },
    { swatch: EMPTY, outline: OUTLINE.rob, label: "Garis teal · ROB di atas normal" },
  ...
  banjir: [
    ...
    { swatch: FLOOD[1], outline: OUTLINE.flood, label: "Perlu dipantau (50–79%)" },
    { swatch: FLOOD[2], outline: OUTLINE.flood, label: "Tinggi (≥80%)" },
  ],
  cekaman: [
    { swatch: DROUGHT[0], outline: OUTLINE.empty, label: "Di bawah pemantauan (<50%)" },
    { swatch: DROUGHT[1], outline: OUTLINE.drought, label: "Perlu dipantau (50–79%)" },
    { swatch: DROUGHT[2], outline: OUTLINE.drought, label: "Tinggi (≥80%)" },
  ],
